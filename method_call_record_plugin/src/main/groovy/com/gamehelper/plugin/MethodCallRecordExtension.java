...
public class MethodCallRecordExtension {
    ...
    /**
     * 方法调用监控（注意需非系统api调用，才能监控的到，
     * 因为插桩是无法插入到系统api的，比如常见的点击回调就是在系统api调用的）
     *
     * @ key :类名  如：android/telephony/TelephonyManager
     * @ list item value :方法名+方法描述
     * 如：getLine1Number()Ljava/lang/String; 支持多个方法
     */
     ...
     * index2=方法描述符，如：  ()Ljava/lang/String; ； 严格按照顺序填入
     */
     ...
     * index2=方法描述符，如：  ()V; ； 严格按照顺序填入
     */
    public static Map<String, List<String>> replaceFieldInvokeMap = new HashMap<>();

    /**
     * 方法进入监控（比如常见的点击事件，调用处是系统api，
     * 这个时候我们就监控接口实现，也就是方法体的进入）
     *
    ...
    /**
     * 不需要插桩的路径，可以传递全路径，也可以传递父级文件夹
     * 例如 ：com/canzhang/asmdemo/sdk/MySdk
     */
    public static List<String> ignorePath = new ArrayList<>();

    /**
     * 是否需要把log 写到本地，如果是true，则会在工程根目录生成
     * method_call_plugin_hook_log.txt  方便查看日志
     */
